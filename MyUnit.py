...
.. moduleauthor:: Chen Sun <chensun@cosmicdiscord.net>
...
class Unit:
    ...
    def __mul__(self, other):
        ...
        # TODO: determine what instance it was, so that NaturalUnit_inst*NaturalUnit_inst
        # doesn't return a Unit_inst
        # DONE: Done. Implemented in the derived classes.

        return Unit(string)

    def __div__(self, other):
        print('hit. division called')
...
class SIUnit(Unit):
    def __init__(self, input):
        ...
        list_symbol = np.array(
            symbols("g, h, hbar, eV, c, kB, Alfa, GN, mu0, eps0, e, T, G, V"))
        list_val = np.array([g_val, h_val, hbar_val, eV_val, c_val, kB_val,
                             Alfa_val, GN_val, mu0_val, eps0_val, e_val, T_val, G_val, V_val])
...
class NaturalUnit(Unit):
    def __init__(self, input):
        # Unit.__init__(self, input)
        # print(super(NaturalUnit, self).__init__)
        super(NaturalUnit, self).__init__(input)
        ...
        g_val = (1.e-3*(299792458)**2/(1.602176565*10**-19))*eV
        ...
        Jy_val = 1.e-26 * J_val/s_val/m_val**2*s_val
        # particle
        ...
        list_symbol = np.array(symbols("h, hbar, c, kB, mu0, eps0, J, e, Alfa,\
                            T, G, V, kg, g, s, m, cm, GN, AlphaMZ, GF, K, A, fm, nm,\
                            cm, km, au, ly, pc, kpc, Mpc, Gpc, year,\
                            keV, MeV, GeV, TeV"))
        list_val = np.array([h_val, hbar_val, c_val, kB_val, mu0_val, eps0_val, J_val, e_val, Alfa_val,
                             T_val, G_val, V_val, kg_val, g_val, s_val, m_val, cm_val, GN_val, AlphaMZ_val, GF_val, K_val, A_val, fm_val, nm_val,
        ...
        # second round of sub
        list_symbol = np.array(symbols("Rsun,Msun,RsgrA,MsgrA,Rearth,Mearth,Jy,sw,\
                             cosCabibbo2,mproton,mneutron,melectron,mmuon,\
                             mpi0,mpiplus,mk0,vev,Mz,NA,Mplr,Mpl,\
                             barn,Om_m,Om_L,Om_b,Om_c,Om_g,Om_nu,h0,\
                             H0,rho_c"))
        list_val = np.array([Rsun_val, Msun_val, RsgrA_val, MsgrA_val, Rearth_val, Mearth_val, Jy_val, sw_val,
    ...
    def __mul__(self, other):
        res = super(NaturalUnit, self).__mul__(other)

        return NaturalUnit(res)

    def __div__(self, other):
        res = super(NaturalUnit, self).__div__(other)
